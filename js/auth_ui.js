...
export function checkStrength(val) {
  ...
  if (val.length === 0) {
    ...
    label.textContent = 'YẾU'; label.style.color = 'var(--red)';
  } else if (strength <= 50) {
    bar.style.background = 'var(--orange)';
    label.textContent = 'TRUNG BÌNH'; label.style.color = 'var(--orange)';
  } else if (strength <= 75) {
    bar.style.background = 'var(--cyan)';
    label.textContent = 'KHÁ'; label.style.color = 'var(--cyan)';
  } else {
    bar.style.background = 'var(--green)';
    label.textContent = 'MẠNH'; label.style.color = 'var(--green)';
  }
}

// 4. HIỂN THỊ OVERLAY THÀNH CÔNG (Xuất hàm này ra để auth.js gọi được)
